...
import { Proposal, ProposalEstado, WorkOrder, WorkOrderEstado } from "../../types/types";
...
const PROPOSAL_STATUS_COLORS: Record<ProposalEstado, { dot: string; text: string; label: string }> = {
  ...
  ACEPTADA:   { dot: "bg-emerald-400", text: "text-emerald-400", label: "Aceptada" },
...
function RecentProposalCard({ proposal }: { proposal: Proposal }) {
  const s = PROPOSAL_STATUS_COLORS[proposal.estado] ?? PROPOSAL_STATUS_COLORS.PENDIENTE;
  return (
      ...
      <ScrollView {...scrollProps} className="flex-1 px-5 pt-4">
        ...
        <View className="flex-row items-center justify-between mb-6">
          <View>
            ...
            <Text className="text-white text-2xl font-bold mt-1">{profile?.nombre}</Text>
          </View>
          {rating != null && rating > 0 && (
            <View className="bg-amber-500/10 border border-amber-500/20 px-3 py-1.5 rounded-full flex-row items-center" style={{ gap: 4 }}>
              <Ionicons name="star" size={14} color="#f59e0b" />
              <Text className="text-amber-400 font-bold text-sm">{rating.toFixed(1)}</Text>
            </View>
          )}
        </View>
        ...
        {loading ? (
          ...
          <View className="flex-row mb-8" style={{ gap: 12 }}>
            <View className="bg-gray-900 border border-gray-800 rounded-2xl flex-1 p-4">
              <Ionicons name="send-outline" size={22} color="#10b981" style={{ marginBottom: 8 }} />
              <Text className="text-white text-2xl font-bold">{pendingProposals.length}</Text>
            ...
            <View className="bg-gray-900 border border-gray-800 rounded-2xl flex-1 p-4">
              ...
              <Text className="text-white text-2xl font-bold">{activeOrders.length}</Text>
            ...
            <View className="bg-gray-900 border border-gray-800 rounded-2xl flex-1 p-4">
              ...
              <Text className="text-white text-2xl font-bold">{completedOrders.length}</Text>
        ...
        <View className="flex-row mb-8" style={{ gap: 12 }}>
          ...
          >
            ...
            <Text className="text-emerald-400 font-bold text-sm">Mis propuestas</Text>
          </TouchableOpacity>
        </View>
        ...
        {!loading && recentProposals.length > 0 && (
          <View className="mb-8">
            <View className="flex-row items-center justify-between mb-4">
              <Text className="text-white text-lg font-bold">Propuestas recientes</Text>
              <TouchableOpacity onPress={() => router.push("/(profesional)/propuestas" as any)}>
        ...
        {!loading && recentOrders.length > 0 && (
          <View className="mb-8">
            <View className="flex-row items-center justify-between mb-4">
              <Text className="text-white text-lg font-bold">Órdenes activas</Text>
              <TouchableOpacity onPress={() => router.push("/(profesional)/ordenes" as any)}>
                <Text className="text-emerald-500 font-medium">Ver todas</Text>
              </TouchableOpacity>
            </View>
            {recentOrders.map((o) => (
        ...
        {!loading && !hasActivity && (
          <View className="bg-gray-900 border border-gray-800 rounded-2xl p-6 items-center mb-8">
            <Ionicons name="rocket-outline" size={40} color="#4b5563" style={{ marginBottom: 12 }} />
            ...
            <Text className="text-gray-400 text-sm text-center leading-5 mb-4">
              Explorá el mercado y enviá propuestas para empezar a recibir órdenes.
            </Text>
            <TouchableOpacity
              onPress={() => router.push("/(profesional)/mercado" as any)}
