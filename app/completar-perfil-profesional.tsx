    ...
    return (
      <SafeAreaView className="flex-1 bg-white dark:bg-gray-950">
        <View className="flex-1 items-center justify-center">
          ...
          <Text className="mt-4 text-slate-500 dark:text-gray-400">Cargando categorías...</Text>
        </View>
      </SafeAreaView>
    );
  }
      ...
      >
        <ScrollView className="flex-1 px-6 py-6">
          ...
          <View className="gap-4">
            {categories?.length === 0 ? (
              <View className="items-center rounded-2xl bg-slate-50 dark:bg-gray-900 p-8">
                <Ionicons
                  name="folder-open-outline"
                  size={48}
                  color="#4b5563"
                />
                <Text className="mt-4 text-center text-slate-500 dark:text-gray-400">
                  ...
                  >
                    <View className="flex-row items-center gap-3">
                      ...
                      <View className="flex-1">
                        ...
                        {cat.descripcion && (
                          <Text className="mt-1 text-xs text-slate-500 dark:text-gray-400">
                            {cat.descripcion}
                          </Text>
                        )}
                      </View>
                  ...
                  {isJobSelected(cat.id) && (
                    <View className="mt-4 flex-row items-center gap-3 border-t border-slate-200 dark:border-gray-800 pt-4">
                      ...
                      <View className="flex-1 flex-row items-center rounded-xl border border-slate-300 dark:border-gray-700 bg-slate-100 dark:bg-gray-800 px-3">
                        ...
                        />
                        <Text className="text-sm text-slate-500 dark:text-gray-400">/hora</Text>
                      </View>
                    </View>
                  )}
                </View>
          ...
          {selectedJobs.length > 0 && (
            <View className="mt-6 rounded-2xl border border-slate-200 dark:border-gray-800 bg-slate-50 dark:bg-gray-900 p-4">
              <Text className="mb-2 text-sm font-medium text-slate-500 dark:text-gray-400">
        ...
        {/* Submit Button */}
        <View className="border-t border-slate-200 dark:border-gray-800 bg-white dark:bg-gray-950 px-6 py-4">
          <Pressable
            accessibilityRole="button"
            ...
            {isSubmitting ? (
              ...
              <Text
                className={`text-base font-semibold ${
                  selectedJobs.length === 0 ? "text-gray-500" : "text-gray-900 dark:text-white"
                }`}
              >
                {selectedJobs.length === 0
